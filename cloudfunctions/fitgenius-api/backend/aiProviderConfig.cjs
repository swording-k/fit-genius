...
function resolveUpstreamModel(requestedModel, config) {
  ...
  const textAliases = new Set(["", "fitgenius-text", "qwen3-omni-flash"]);
  const visionAliases = new Set(["fitgenius-vision", "fitgenius-video", "qwen-vl-max"]);
...
module.exports = { resolveAIProviderConfig, resolveUpstreamModel };
